...
class Node:
    ...
    def __init__(
        ...
        tokens=None,
        log_rnd=None,
        log_policy_step=None,
        log_pretrained_step=None,
        parentNode=None,
        childNodes=None,
        totalReward=None,
        timestep=None,
        ...
        if totalReward is not None:
            self.totalReward = (
                totalReward  # potential reward of the node based on generated children
            )
        else:
    ...
    def calcSelectScore(self):
        ...
        return normRewards + (
            scaling
            * self.log_policy_step.detach().cpu().item()
            * np.sqrt(self.parentNode.visits)
            / self.visits
        )
...
class MCTS:
    def __init__(
        ...
        # score functions - use MolScoringFunctions for molecules
        self.rewardFunc = MolScoringFunctions(score_func_names, device=args.device)

        self.iter_num = 0

        self.reward_log = []  # stores scalarized total rewards
        self.logrnd_log = []
        # stores each objective dynamically
        ...
        # for molecules - use SafeAnalyzer from policy_model
    ...
    def reset(self, resetTree):
        ...
        # reset logs for each objective dynamically
        self.valid_fraction_log = []
    ...
    def forward(self, resetTree=False):
        ...
        print(
            f"[MCTS FORWARD] Starting MCTS with {self.num_iter} iterations, buffer_size={self.buffer_size}"
        )

        while self.iter_num < self.num_iter:
            self.iter_num += 1
            print(
                f"[MCTS FORWARD] Iteration {self.iter_num}/{self.num_iter}, buffer_size={len(self.buffer)}"
            )
            ...
            print(f"[MCTS FORWARD] After iteration {self.iter_num}, buffer_size={len(self.buffer)}")

        print(
            f"[MCTS FORWARD] Completed {self.num_iter} iterations. Final buffer_size={len(self.buffer)}"
        )
    ...
    def _debug_buffer_decision(self, sv, reason, extra=None):
        if extra is None:
            extra = {}
        print(
            f"[BUFFER] reason={reason} sv={np.round(sv,4)} "
    ...
    def updateBuffer(self, x_final, log_rnd, score_vectors, childSequences):
        B = x_final.shape[0]
        print(
            f"[UPDATE_BUFFER] Attempting to add {B} items to buffer (current size: {len(self.buffer)})"
        )
        ...
        print(
            f"[UPDATE_BUFFER] Result: {inserted_count} inserted, {rejected_count} rejected, final buffer_size={len(self.buffer)}"
        )
    ...
    def consolidateBuffer(self):
        ...
        """
        print(f"[CONSOLIDATE] Buffer size: {len(self.buffer)}")
        print(
            f"[CONSOLIDATE] Buffer contents: {[item.get('seq', 'NO_SEQ')[:50] for item in self.buffer[:5]]}"
        )
        ...
        if len(x_final) == 0:
            print(f"[CONSOLIDATE] ERROR: Buffer is empty! Cannot stack tensors.")
            print(
                f"[CONSOLIDATE] This means no valid sequences were added to buffer during MCTS iterations."
            )
            raise RuntimeError(
                "Buffer is empty - no valid sequences were generated during MCTS. "
                "Check if sequences are being validated correctly and if buffer update logic is working."
            )
        ...
        print(f"[CONSOLIDATE] Successfully stacked {len(x_final)} items")
            ...
            if currNode.parentNode is not None:
                ...
                with torch.no_grad():
                    with self.timer.section("select.compute_log_policy"):
                        updated_log_policy_step = self.policy_model.compute_log_policy(
                            parent_tokens, child_tokens, t=t, dt=dt
                        )
    ...
    def expand(self, parentNode, eps=1e-5):
        ...
        print(
            f"[EXPAND] Starting expansion: parent_timestep={parentNode.timestep}, num_rollout_steps={num_rollout_steps}, num_children={num_children}"
        )
        print(f"[EXPAND] Parent token shape: {x.shape}, mask_index={self.mask_index}")
        with torch.no_grad():
            with self.timer.section("expand.batch_mcts_reverse_step"):
                _, x_children, child_log_policy_step, child_log_pretrained_step = (
                    self.policy_model.batch_mcts_reverse_step(
                        token_array=x,
                        t=t,
                        dt=dt,
        ...
        print(f"[EXPAND] After batch_mcts_reverse_step: x_children shape={x_children.shape}")

        # compute weight of the step (num_children, 1)

        child_log_rnd = (parent_log_rnd + (child_log_pretrained_step - child_log_policy_step)).to(
            self.device
        )
        ...
        with self.timer.section("expand.rollout_total"):
            for i in range(1, num_rollout_steps):
                t = rollout_t[parentNode.timestep + i] * torch.ones(
                    num_children, 1, device=self.device
                )

                with torch.no_grad():
                    _, x_next, log_policy_step, log_pretrained_step = (
                        self.policy_model.mcts_reverse_step(
                            x_rollout, t=t, dt=dt, pretrained=self.pretrained
                        )
                    )
            ...
            with torch.no_grad():
                with self.timer.section("expand.noise_removal"):
                    log_p, x_next, log_policy_step, log_pretrained_step = (
                        self.policy_model.mcts_noise_removal(
                            x_rollout, t=t, dt=dt, pretrained=self.pretrained
                        )
                    )
        ...
        with self.timer.section("expand.decode"):
            ...
            print(
                f"[EXPAND] Decoded {len(childSequences)} sequences. First few: {[s[:50] for s in childSequences[:3]]}"
            )
        ...
        with self.timer.section("expand.filter_is_valid"):
            print(f"[EXPAND] Checking validity of {num_children} sequences...")
            valid_count = 0
            invalid_count = 0
            for i in range(num_children):
                ...
                is_valid = self.analyzer.is_valid(childSeq)
                if is_valid:
                    valid_count += 1
                    ...
                    invalid_count += 1
                    if invalid_count <= 3:  # Print first few invalid sequences for debugging
                        print(f"[EXPAND] Invalid sequence {i}: {childSeq[:100]}")
                    childTokens = {
                        "seqs": x_children[i].to(dtype=torch.long),
            ...
            print(
                f"[EXPAND] Validity check: {valid_count} valid, {invalid_count} invalid out of {num_children} total"
            )
        ...
        if len(validSequences) > 0:
            ...
            with self.timer.section("expand.scoring_functions"):
                score_dict = self.rewardFunc(input_seqs=validSequences)
                # Convert dict to array: (num_sequences, num_objectives)
                score_vectors = np.array([score_dict[name] for name in self.score_func_names]).T

            # Compute average scores per objective (across sequences)
            average_scores = np.mean(score_vectors, axis=0)

            # Log scores dynamically
            for idx, name in enumerate(self.score_func_names):
                self.score_logs[name].append(average_scores[idx])
        else:
            # set the values added to log as 0s if there are no valid sequences
            for name in self.score_func_names:
                self.score_logs[name].append(0.0)
            score_vectors = np.zeros((0, self.num_obj))
        ...
        with self.timer.section("expand.update_buffer"):
            traj_log_rnds, scalar_rewards = self.updateBuffer(
                valid_x_final, valid_traj_log_rnd, score_vectors, validSequences
            )
        ...
        for i in range(len(score_vectors)):
            ...
            childTokens = {
                "seqs": valid_x_children[i].to(dtype=torch.long),
                "attention_mask": attn_mask,
            }
            parentNode.addChildNode(
                tokens=childTokens,
        ...
        # debugging
        print(
            f"[EXPAND] iter={self.iter_num} parent_t={parentNode.timestep} "
            f"num_children={num_children} valid={len(validSequences)} any_mask={any_mask_global}"
        )
        if score_vectors is not None:
            print(
                f"[SCORES] min={np.min(score_vectors,0)} max={np.max(score_vectors,0)} "
        ...
        allChildReward = allChildReward / len(
            validSequences
        )  # normalize by number of valid children
