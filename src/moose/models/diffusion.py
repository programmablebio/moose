...
import gc
import pandas as pd
import moose.utils.utils as utils
from torch.optim.lr_scheduler import _LRScheduler

from moose.tokenizer.my_tokenizers import SMILES_SPE_Tokenizer
import moose.models.noise_schedule as noise_schedule
import moose.models.roformer as roformer

# from moose.utils.app import PeptideAnalyzer
...
from moose.utils.utils_data import get_tokenizer
from moose.utils.utils_save import clean_checkpoint, fast_forward_info
from moose.utils.ema import ExponentialMovingAverage
from moose.utils.utils_moco import AntitheticUniformTimeDistribution
...
def _sample_categorical_gradient(categorical_probs, temp=1.0):
    ...
    output = torch.nn.functional.softmax(
        (torch.log(categorical_probs) - torch.log(gumbel_norm)) / temp, 2
    )
    return output
...
def sample_batched_categorical(categorical_probs, batch_size):
    ...
    gumbel_noise = (
        -torch.log(-torch.log(torch.rand(batch_size, sequence_length, vocab_size) + 1e-10) + 1e-10)
    ).to(categorical_probs.device)
    noisy_scores = (
        torch.log(categorical_probs) + gumbel_noise
    )  # add Gumbel noise to log probabilities

    # select the highest score (most likely category after Gumbel noise)
    sampled_sequences = noisy_scores.argmax(dim=-1).to(
        dtype=torch.long
    )  # shape: (m, sequence_length)
...
def sample_batched_top_k(categorical_probs, batch_size, k):
    ...
    gumbel_noise = -torch.log(
        -torch.log(torch.rand(batch_size, sequence_length, vocab_length) + 1e-10) + 1e-10
    ).to(categorical_probs.device)
    noisy_scores = (
        torch.log(categorical_probs[None, :, :]) + gumbel_noise
    )  # Shape: (m, sequence_length, vocab_length)

    # Get the top-k categories based on noisy scores
    top_k_scores, top_k_indices = torch.topk(
        noisy_scores, k, dim=-1
    )  # Shape: (m, sequence_length, k)

    # Convert top-k scores back to probabilities and normalize
    top_k_probs = torch.softmax(top_k_scores, dim=-1).to(
        categorical_probs.device
    )  # Shape: (m, sequence_length, k)

    # Sample randomly from the top-k probabilities
    sampled_indices_in_top_k = (
        torch.multinomial(top_k_probs.reshape(-1, k), num_samples=1)
        .squeeze(-1)
        .to(categorical_probs.device)
    )
    sampled_indices_in_top_k = sampled_indices_in_top_k.view(batch_size, sequence_length).to(
        categorical_probs.device
    )  # Shape: (batch_size, sequence_length)

    # Map sampled indices back to the original vocabulary indices
    sampled_sequences = (
        torch.gather(top_k_indices, -1, sampled_indices_in_top_k.unsqueeze(-1))
        .squeeze(-1)
        .to(categorical_probs.device)
        .to(dtype=torch.long)
    )
...
class SafeAnalyzer:
    def is_valid(self, safe_str: str) -> bool:
        try:
...
class Diffusion(L.LightningModule):
    def __init__(
        ...
        if hasattr(self.config.model, "to_dict"):
            self.backbone = BertForMaskedLM(BertConfig.from_dict(self.config.model.to_dict()))
        else:
        ...
        if mode == "finetune":
            ...
            total_layers = len(self.backbone.bert.encoder.layer)
            unfreeze_layers = 8
            for p in self.backbone.parameters():
                p.requires_grad = False
            for i in range(min(unfreeze_layers, total_layers)):
                for p in self.backbone.bert.encoder.layer[-i - 1].parameters():
        ...
        if self.config.training.ema > 0:
            self.ema = ExponentialMovingAverage(
                self.backbone.parameters(), decay=self.config.training.ema
            )
        else:
            self.ema = None
        ...
        self.training_scheme = getattr(self.config.training, "scheme", "tr2d2")
        if self.training_scheme == "genmol":
            ...
            prior = DiscreteMaskedPrior(num_classes=self.vocab_size, mask_dim=self.mask_index)
            noise_transform = LogLinearExpNoiseTransform()
            self.mdlm = MDLM(
                time_distribution=time_distribution,
                prior_distribution=prior,
                noise_schedule=noise_transform,
            )
        else:
            self.mdlm = None

        ### WHAT ARE EQUIVALENT METRICS FOR GENMOL?
    ...
    def sample_finetuned_with_rnd(self, args, reward_model, pretrained, eps=1e-5):
        ...
        B = args.batch_size
        x_rollout = self.sample_prior(B, args.seq_length).to(self.device)
        ...
        for i in range(num_steps):
            ...
            log_p, x_next, log_policy_step, log_pretrained_step = self.mcts_reverse_step(
                x_rollout, t=t, dt=dt, pretrained=pretrained
            )
        ...
        childSequences = self.tokenizer.batch_decode(x_rollout, skip_special_tokens=True)

        # compute multi-objective rewards for molecules
        valid_x_final = []
        validSequences = []
        valid_log_rnd = []
        ...
        for i in range(B):
            ...
            if self.analyzer.is_valid(childSeq):
        ...
        if len(validSequences) > 0:
            score_dict = reward_model(input_seqs=validSequences)
            # Convert dict to array: (num_sequences, num_objectives)
            # Stack arrays in order of score_func_names, then transpose
            score_vectors = np.array([score_dict[name] for name in reward_model.score_func_names]).T
            scalar_rewards = np.sum(
                score_vectors, axis=1
            )  # Sum across objectives for each sequence
            scalar_rewards = torch.as_tensor(
                scalar_rewards, dtype=torch.float32, device=self.device
            )
            ...
            # No valid sequences
            scalar_rewards = torch.zeros(0, dtype=torch.float32, device=self.device)
            valid_log_rnd = torch.zeros(0, dtype=torch.float32, device=self.device)
            valid_x_final = torch.zeros((0, args.seq_length), dtype=torch.long, device=self.device)
            log_rnd = valid_log_rnd
    ...
    def sample_finetuned(self, args, reward_model, batch_size=None, dataframe=False, eps=1e-5):
        ...
        num_steps = args.total_num_steps
        x_rollout = self.sample_prior(batch_size, args.seq_length).to(self.device, dtype=torch.long)
        ...
        if len(validSequences) > 0:
            # add scores to log
            score_dict = reward_model(input_seqs=validSequences)
            # Compute mean per objective (across sequences)
            scores = {name: np.mean(score_dict[name]) for name in score_func_names}

            if dataframe:
                # Create DataFrame with per-sequence scores
                df_data = {"SAFE Sequence": validSequences}
                for name in score_func_names:
                    df_data[name] = score_dict[name]
                df = pd.DataFrame(df_data)
                return x_rollout, scores, valid_fraction, df
        else:
            # No valid sequences
            scores = {name: 0.0 for name in score_func_names}
            if dataframe:
                df_data = {"SAFE Sequence": []}
                for name in score_func_names:
                    df_data[name] = [0.0]
                df = pd.DataFrame(df_data)
                return x_rollout, scores, valid_fraction, df

        # Return without DataFrame
        return x_rollout, scores, valid_fraction
    ...
    def mcts_reverse_step(self, token_array, t, dt, pretrained, p_x0=None, attn_mask=None):
        ...
        with torch.no_grad():
            ...
            unmasked_this_step = (changed_mask & (x_next != self.mask_index)).to(
                log_pre_token.dtype
            )
    ...
    def batch_mcts_reverse_step(
        self, token_array, t, dt, batch_size, pretrained, p_x0=None, attn_mask=None
    ):
        torch.cuda.empty_cache()
        self.backbone.eval()
        self.noise.eval()
        ...
        if self.config.mcts.sampling == 0:
            ...
            x_changed = sample_batched_top_k(
                q_xs.to(self.device), batch_size, self.config.mcts.sampling
            )
        ...
        with torch.no_grad():
            ...
            changed_mask = ~copy_flag
            # mask of tokens that were unmasked in this step
            unmasked_this_step = (changed_mask & (x_children != self.mask_index)).to(
                log_pre_token.dtype
            )
        ...
        log_policy_token = log_p.gather(-1, x_children.unsqueeze(-1)).squeeze(
            -1
        )  # (B, L) probability of each chosen token
    ...
    def compute_invalid_loss(self, logits, k=None, temp=None):
        ...
        sampled_sequences = self.tokenizer.batch_decode(batch_token_ids, skip_special_tokens=True)
        ...
        penalties = torch.tensor(
            ...
            [1 if not self.analyzer.is_valid(seq) else 0 for seq in sampled_sequences],
            dtype=torch.float32,
            device=self.device,
        )
        # print(penalties)
        ...
        sampled_probs = (
            ...
            .gather(dim=-1, index=batch_token_ids.unsqueeze(-1))
            .squeeze(-1)
            .to(self.device)
        )
    ...
    def compute_diffusion_loss(self, model_output, xt, x0, t):
        ...
        L_vb_masked = term_1_coef * (term_1_log_numerator - term_1_log_denom) + term_2_coef * (
            term_2_log_numerator - term_2_log_denom
        )
    ...
    def _forward_pass_diffusion(self, x0, attn_mask, bond_mask=None, mask=None):
        ...
        model_output = self.forward(
            zt, attn_mask=attn_mask.to(self.device), sigma=time_conditioning
        ).to(self.device)
        ...
        log_p_theta = (
            torch.gather(input=model_output, dim=-1, index=x0[:, :, None])
            .squeeze(-1)
            .to(self.device)
        )  # (B, L)
        ...
        if self.config.noise.state_dependent and (bond_mask is not None):
            ...
            return ((-log_p_theta * (dsigma / torch.expm1(sigma))[:, None]) + invalid_loss).to(
                self.device
            )
    ...
    def _loss(self, x0, attn_mask, bond_mask=None, mask=None):
        ...
        # return losses
        return Loss(
            loss=token_nll.to(self.device),
            nlls=nlls.to(self.device),
    ...
    def _compute_loss(self, batch, prefix, bond_mask=None):
        ...
        if prefix == "train":
            self.train_metrics.update(losses.nlls.to(self.device), losses.attn_mask.to(self.device))
            metrics = self.train_metrics
        elif prefix == "val":
            self.valid_metrics.update(losses.nlls.to(self.device), losses.attn_mask.to(self.device))
    ...
    def forward(self, zt, attn_mask, sigma):
        ...
        ## Adding some safeguards since we're working with a different backbone now
        with torch.cuda.amp.autocast(dtype=torch.float32):
            # logits = self.backbone(zt, attn_mask).to(self.device)
            outputs = self.backbone(
                input_ids=zt.to(self.device), attention_mask=attn_mask.to(self.device)
            )
            logits = outputs.logits if hasattr(outputs, "logits") else outputs

        return self.subs_parameterization(logits, zt.to(self.device))
    ...
    def subs_parameterization(self, logits, zt):
        ...
        # Clone logits first to avoid in-place operations on the original tensor
        logits = logits.clone()

        # Step 1: Set mask token logits to -infinity (in-place on clone is OK)
        logits[..., self.mask_index] = logits[..., self.mask_index] + self.neg_infinity

        # Step 2: Apply log_softmax (creates new tensor, breaks in-place chain)
        logits = torch.log_softmax(logits, dim=-1)

        # Step 3: Clone again before modifying unmasked positions
        # This is critical: after log_softmax, we have a new tensor in the graph.
        # Cloning it creates a leaf node that we can safely modify in-place.
        logits = logits.clone()

        unmasked_indices = zt != self.mask_index  # shape: [batch, seq_length]
        batch_idx, seq_idx = torch.where(unmasked_indices)  # Get explicit indices
        tokens = zt[batch_idx, seq_idx]  # Get the tokens at those positions

        # Now in-place operations on the cloned tensor are safe
        if len(batch_idx) > 0:
            logits[batch_idx, seq_idx] = self.neg_infinity
            logits[batch_idx, seq_idx, tokens] = 0.0
    ...
    def _sample(self, num_steps=None, eps=1e-5, x_input=None):
        ...
        for i in range(num_steps):
            ...
            if self.sampler == "ddpm":
                ...
                p_x0_cache, x_next = self.cached_reverse_step(
                    x, t, dt, p_x0=p_x0_cache, attn_mask=attn_mask
                )
        ...
        if self.config.sampling.noise_removal:
            ...
            if self.sampler == "analytic":
                ...
                x = (
                    self.forward(x, attn_mask=attn_mask, sigma=time_conditioning)
                    .argmax(dim=-1)
                    .to(self.device)
                )
                # print(self.tokenizer.decode(x.squeeze()))
    ...
    def get_score(self, zt, sigma, attn_mask=None):
        ...
        unmasked_score = torch.scatter(
            unmasked_score, -1, zt[..., None], torch.zeros_like(unmasked_score[..., :1])
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.training_scheme == "genmol":
            loss = self._training_step_genmol(batch)
        elif self.config.vocab == "old_smiles" or self.config.vocab == "new_smiles":
    ...
    def _training_step_genmol(self, batch):
        input_ids = batch["input_ids"].to(self.device)
        attention_mask = batch["attention_mask"].to(self.device)

        if self.mdlm is None:
            raise RuntimeError("GenMol training scheme selected but MDLM is not initialized.")

        t = self.mdlm.sample_time(input_ids.shape[0]).to(self.device)
        xt = self.mdlm.forward_process(input_ids, t).to(self.device)

        with torch.cuda.amp.autocast(dtype=torch.float32):
            outputs = self.backbone(input_ids=xt, attention_mask=attention_mask)
            logits = outputs.logits if hasattr(outputs, "logits") else outputs

        global_mean = getattr(self.config.training, "global_mean_loss", False)
        loss = self.mdlm.loss(
            logits, input_ids, xt, t, mask=attention_mask, global_mean=global_mean
        )
