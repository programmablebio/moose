...
class Hemolysis:
    ...
    def __init__(self, tokenizer, base_path, device=None, emb_model=None):
        ...
        self.predictor = xgb.Booster(
            model_file=f"{base_path}/TR2-D2/tr2d2-pep/scoring/functions/classifiers/hemolysis-xgboost.json"
        )
        self.emb_model = (
            emb_model
            if emb_model is not None
            else AutoModelForMaskedLM.from_pretrained("aaronfeller/PeptideCLM-23M-all")
            .roformer.to(device)
            .eval()
        )
...
def unittest():
    hemo = Hemolysis()
    seq = [
        "[te]NCC(=O)N[C@H](CS)C(=O)N[C@@H](CO)C(=O)NCC(=O)N[C@@H](CC1=CN=C-N1)C(=O)N[C@@H](CC(=O)N)C(=O)N[C@@H](CC(=CN2)C1=C2C=CC=C1)C(=O)N[C@@H](c1ccc(cc1)F)C(=O)N[C@@H]([C@H](CC)C)C(=O)N[C@@H](CCCO)C(=O)N[C@@H](CC1=CN=C-N1)C(=O)N[C@@H](CCC(=O)O)C(=O)N[C@@H](CO)C(=O)O"
    ]
