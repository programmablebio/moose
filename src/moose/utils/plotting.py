...
def plot_data_with_distribution_seaborn(
    log1, log2=None, save_path=None, label1=None, label2=None, title=None
):
    """
    ...
    # Handle both scalars and iterables: if element is scalar, wrap it in a list
    def ensure_list(item):
        """Convert scalar to list, or return list if already iterable (but not string)"""
        # Check if it's a numpy scalar or Python scalar
        if isinstance(item, (np.number, int, float, complex, bool)):
            return [item]
        # Check if it's a string (which is iterable but we want to treat as single item)
        if isinstance(item, (str, bytes)):
            return [item]
        # Check if it's a numpy array (even 0-d arrays should be converted)
        if isinstance(item, np.ndarray):
            if item.ndim == 0:
                return [item.item()]
            else:
                return item.tolist()
        # Try to iterate - if it works, it's an iterable
        try:
            return list(item)
        except (TypeError, ValueError):
            # It's a scalar, wrap in list
            return [item]

    log1 = [ensure_list(scores) for scores in log1]
    if log2 is not None:
        log2 = [ensure_list(scores) for scores in log2]
