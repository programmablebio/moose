...
class DilatedCNN(nn.Module):
    ...
    def forward(self, protein_seq_enc):
        # pdb.set_trace()
        protein_seq_enc = protein_seq_enc.transpose(
            1, 2
        )  # protein_seq_enc's shape: B*L*d_model -> B*d_model*L
...
class ReciprocalLayerwithCNN(nn.Module):
    ...
    def forward(self, sequence_enc, protein_seq_enc):
        ...
        prot_enc, seq_enc, prot_seq_attention, seq_prot_attention = self.reciprocal_attention_layer(
            prot_enc, seq_enc, seq_enc, prot_enc
        )
