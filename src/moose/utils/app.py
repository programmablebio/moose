...
class PeptideAnalyzer:
    ...
    def identify_residue(self, segment):
        ...
        if (
            any(
                [
                    ...
                    (
                        segment.get("bond_after", "").startswith(f"N{n}C(=O)")
                        and "CCC" in content
                        and any(
                            f"[C@@H]{n}" in content or f"[C@H]{n}" in content for n in "123456789"
                        )
                    )
                    for n in "123456789"
                ]
            ...
            or any(
                [
                    ...
                    (
                        content == f"CCC[C@H]{n}"
                        and segment.get("bond_before", "").startswith(f"C(=O)N{n}")
                    )
                    or (
                        content == f"CCC[C@@H]{n}"
                        and segment.get("bond_before", "").startswith(f"C(=O)N{n}")
                    ...
                    (f"N{n}CCC[C@H]{n}" in content) or (f"N{n}CCC[C@@H]{n}" in content)
                    for n in "123456789"
                ]
            )
        ):
        ...
        if re.search(
            r"c[0-9]c\[nH\]c[0-9]ccccc[0-9][0-9]", content
        ) and "c[nH]c" in content.replace(" ", ""):
            return "Trp", mods
        ...
        if ("C[C@H](CC)" in content or "C[C@@H](CC)" in content) and not any(
            p in content for p in ["CC(C)", "CCCC", "CCC(C)"]
        ):
            return "Abu", mods

        # Pipecolic acid (Pip) - 6-membered ring like Pro
        if ("N3CCCCC3" in content or "CCCCC3" in content) and (
            "C[C@H]" in content or "C[C@@H]" in content
        ):
            return "Pip", mods
        ...
        if (
            "[C@H](C(C)C)" in content  # With outer parentheses
            or "[C@@H](C(C)C)" in content  # With outer parentheses
            or "[C@H]C(C)C" in content  # Without outer parentheses
            or "[C@@H]C(C)C" in content
        ):  # Without outer parentheses
            if not any(
                p in content for p in ["CC(C)C[C@H]", "CC(C)C[C@@H]"]
            ):  # Still check not Leu
                return "Val", mods
        ...
        if "[C@H](C)" in content or "[C@@H](C)" in content:
            if not any(
                p in content for p in ["C(C)C", "COC", "CN(", "C(C)O", "CC[C@H]", "CC[C@@H]"]
            ):
                return "Ala", mods
        ...
        if (
            "[C@@H]([C@@H](C)O)" in content
            or "[C@H]([C@H](C)O)" in content
            or "[C@@H](C)O" in content
            or "[C@H](C)O" in content
        ):
            return "Thr", mods
...
def annotate_cyclic_structure(mol, sequence):
    ...
    draw.text((1000, 100), seq_text, font=small_font, fill="black", anchor="mm")
...
def create_enhanced_linear_viz(sequence, smiles):
    ...
    for i in range(num_residues):
        ...
        rect = patches.Rectangle(
            (x_pos - 0.3, y_pos - 0.2), 0.6, 0.4, facecolor="lightblue", edgecolor="black"
        )
        ...
        # Add residue label
        ax_struct.text(x_pos, y_pos - 0.5, residues[i], ha="center", va="top", fontsize=14)
    ...
    if sequence.startswith("cyclo("):
        ...
        ax_struct.text(5, y_pos + 0.3, "Cyclic Connection", ha="center", color="red", fontsize=14)
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if AllChem.EmbedMolecule(test_mol, params) == 0:
                    res = AllChem.UFFOptimizeMolecule(
                        test_mol,
                        maxIters=2000,
                        vdwThresh=10.0,
                        confId=0,
...
def process_input(
    ...
    if smiles_input:
        ...
        try:
            ...
            one_letter = "".join(
                analyzer.three_to_one.get(aa.split("(")[0], "X") for aa in sequence_parts
            )
            ...
            return (
                ...
                img_cyclic,
                img_linear,
                structure_files if structure_files else None,
            )
        ...
        try:
            ...
            for line in content.splitlines():
                ...
                if smiles:
                    ...
                    is_cyclic, peptide_cycles, aromatic_cycles = analyzer.is_cyclic(smiles)
                    sequence = (
                        f"cyclo({'-'.join(sequence_parts)})"
                        if is_cyclic
                        else "-".join(sequence_parts)
                    )
