...
1. `conda env create -p /scratch/pranamlab/shared/miniconda3/envs/moose/ -f new_environment.yml`
2. `pip install -e .`
3. `conda remove -y pytorch torchvision torchaudio pytorch-mutex`
4. `conda install -c nvidia cuda-toolkit=12.4`
5. `pip install torch==2.6.0+cu124 torchvision==0.21.0+cu124 torchaudio==2.6.0+cu124 --index-url https://download.pytorch.org/whl/cu124`
6. 
```
python -m pip uninstall -y fsspec 
python -m pip install --no-cache-dir --force-reinstall fsspec==2024.2.0
```
7. `pip install black`